...
class Geometry:
    ...
    def __init__(self, points, lines, surfaces, point_colors):
        self.points = points
        self.lines = lines
    ...
    def tetraeder():
        ...
        point_colors = np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255], [128, 128, 0]])
        ...
        return Geometry(np.array(centered_points), np.array(lines), np.array(surfaces), np.array(point_colors))
    ...
    def outside_coordinate_system(self, coordinate_system):
        ...
        return Geometry(transformed_points, self.lines.copy(), self.surfaces.copy(), self.point_colors.copy())

    def draw(self, screen, z_buffer):
        for l in self.lines:
            start = project_on_screen(self.points[l[0]])
            finish = project_on_screen(self.points[l[1]])

            line(screen, start, finish, [255, 0, 0])
        ...
        for i in range(len(self.surfaces)):
            ...
            a_color = self.point_colors[s[0]]
            b_color = self.point_colors[s[1]]
            c_color = self.point_colors[s[2]]

            project_triangle_filled(screen, z_buffer, a, b, c, a_color, b_color, c_color)
...
def project_triangle_filled(screen, z_buffer, world_a, world_b, world_c, vertex_color_a, vertex_color_b, vertex_color_c):
    #@TODO prove and then explain why the homogenous component w can be used like this
    a = project_on_screen(world_a)
    b = project_on_screen(world_b)
    c = project_on_screen(world_c)
    ...
    triangle_origin = a
    triangle_origin_w = a_w
    triangle_x_axis, x_count = unit_axis(triangle_origin, b)
    x_z_increase = world_b[2] - world_a[2]
    x_w_increase = (b_w - a_w)
    triangle_x_w =  x_w_increase / x_count
    triangle_y_axis, y_count = unit_axis(triangle_origin, c)
    y_z_increase = world_c[2] - world_a[2]
    y_w_increase = (c_w - a_w)
    triangle_y_w = y_w_increase / y_count

    vertex_color_origin = vertex_color_a
    vertex_color_x_direction = vertex_color_b - vertex_color_a
    vertex_color_y_direction = vertex_color_c - vertex_color_a
    ...
    for x in range(int(x_count)):
        for y in range(int(y_count)):
            ...
            w = triangle_origin_w + x * triangle_x_w + y * triangle_y_w
            z = 1./w
            if z_buffer[screen_x, screen_y] <= z:
                continue

            z_buffer[screen_x, screen_y] = z
...
def loop(elapsed_time, screen, mouse_x, mouse_y, mouse_is_pressed, mouse_went_down, mouse_went_up):
    ...
    z_buffer = np.ones([640, 480]) * sys.float_info.max
    transformed_cube.draw(screen, z_buffer)
