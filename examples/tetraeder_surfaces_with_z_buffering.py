...
class Geometry:
    ...
    def __init__(self, points, lines, surfaces, surface_colors):
        self.points = points
        self.lines = lines
    ...
    def tetraeder():
        ...
        surfaces = []
        surface_colors = np.array([[255, 0, 0], [0, 255, 0], [0, 0, 255], [128, 128, 0]])
        ...
        return Geometry(np.array(centered_points), np.array(lines), np.array(surfaces), np.array(surface_colors))
    ...
    def outside_coordinate_system(self, coordinate_system):
        ...
        return Geometry(transformed_points, self.lines.copy(), self.surfaces.copy(), self.surface_colors.copy())
    ...
    def draw(self, screen, z_buffer):
        ...
        for i in range(len(self.surfaces)):
            s = self.surfaces[i]
            color = self.surface_colors[i]

            a = self.points[s[0]]
            b = self.points[s[1]]
            c = self.points[s[2]]

            project_triangle_filled(screen, z_buffer, a, b, c, color)
...
def project_triangle_filled(screen, z_buffer, world_a, world_b, world_c, color):
    ...
    triangle_x_w = (b_w - a_w) / x_count
    triangle_y_axis, y_count = unit_axis(triangle_origin, c)
    triangle_y_w = (c_w - a_w) / y_count
